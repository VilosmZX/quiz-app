...
router.post("/questions/add", (req, res) => {
  ...
  if (!data.question)
    return res
      .status(400)
      .json({ error: "Harap masukan pertanyaan yang benar!" });
  if (!data.answers)
    return res.status(400).json({ error: "Masukan pilihan ganda minimal 4" });
  if (data.answers.length < 2)
    return res
      .status(400)
      .json({ error: "Answer harus lebih dari atau sama dengan 4" });
  for (const answer of data.answers) {
    if (!("isCorrect" in answer) || !("value" in answer))
      return res
        .status(200)
        .json({ error: "Ada jawaban yang tidak memiliki property isCorrect" });
  }
  prisma.question
    .create({
      data: {
